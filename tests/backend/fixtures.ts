...
import { type StoredAnswer, type AnswerMap } from "../../src/lib/backend/answers";
import { type AnswerPayload } from "../../src/lib/client-api";

export function completeAnswerMap() {
  return completeAnswers().answerMap;
}

export function completeStoredAnswers() {
  return completeAnswers().storedAnswers;
}

export function completeAnswerPayloads(sessionId: string, writeToken: string) {
  return completeAnswers().storedAnswers.map((answer): AnswerPayload => {
    const metadata = QUESTION_METADATA[answer.questionId];
...
export function completeAnswers(options: { straightline?: boolean } = {}) {
  const answerMap: AnswerMap = {};
  const storedAnswers: StoredAnswer[] = [];

  for (const scenarioId of SCENARIO_IDS) {
    SCENARIO_RATING_QUESTION_IDS.forEach((questionId, index) => {
      setNumber(questionId, options.straightline ? 4 : (index % 7) + 1, scenarioId);
    });
  }
  ...
  setText("F2_least_convincing_evidence", "The system says it has feelings.");
  setText(
  ...
  return { answerMap, storedAnswers };
  ...
  function setNumber(
    ...
    const answerKey = answerInstanceId(questionId, scenarioId);
    answerMap[answerKey] = value;
    storedAnswers.push({
      answerKey,
      questionId,
  ...
  function setList(questionId: QuestionId, value: string[]) {
    const answerKey = answerInstanceId(questionId);
    answerMap[answerKey] = value;
    storedAnswers.push({
      answerKey,
      questionId,
